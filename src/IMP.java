...
class IMP implements MouseListener{
	...
	private void fun2()			
	{
		...
		for(int i = 0; i < height; i++) {
			for(int j = 0; j < width; j++) {   
				int rgbArray[] = new int[4];
				rgbArray = getPixelArray(picture[i][j]);		//get three ints for R, G and B
				picture[i][j] = getPixels(rgbArray);			//take three ints for R, G, B and put them back into a single int
			}
		}
		resetPicture();
	}
